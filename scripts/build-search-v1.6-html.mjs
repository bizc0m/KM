...
const outputFile = join(root, "search-v1.6.html");
const includeDirs = ["watch"];
const includeFiles = [];
const internalPattern = /\b(bizc0m|charte\s+ia|nightintel|night\s*intel|process:|theme:|km:|###dev|watch:|appel canonique|relations|fichiers touches|rollback)\b/i;
...
<main class="app">
  ...
  <footer class="footer"><span><b>KM Search v1.6</b> · HTML autonome · vue publique nettoyee</span><span>${index.length} fiches · ${generatedAt}</span></footer>
</main>
...
function cardHtml(item){const active=item.id===activeId, sel=selected.has(item.id);return '<article class="news-card '+(active?"active":"")+' '+(sel?"selected":"")+'" data-id="'+esc(item.id)+'"><div class="check" data-action="select" title="Selectionner"></div><div><div class="meta"><span class="tag '+statusClass(item.status)+'">'+esc(item.status)+'</span><span class="tag ai">'+esc(item.bucket)+'</span>'+item.tags.slice(0,4).map(t=>'<span class="tag">'+highlight(t)+'</span>').join("")+'</div><h3 class="news-title" data-action="open">'+highlight(item.title)+'</h3></div></article>'}
...
byId("copySelection").onclick=()=>{const items=KM_INDEX.filter(i=>selected.has(i.id));copy("# KM Search v1.6 selection\\n\\n"+(items.length?items:[activeItem()]).map(itemMarkdown).join("\\n")).then(()=>toast("Selection copiee"))};
byId("exportSelection").onclick=()=>{const items=KM_INDEX.filter(i=>selected.has(i.id));download("km-search-v1.6-selection.md","# KM Search v1.6 selection\\n\\n"+(items.length?items:[activeItem()]).map(itemMarkdown).join("\\n"))};
byId("exportJson").onclick=()=>download("km-search-v1.6-index.json",JSON.stringify(KM_INDEX,null,2));
