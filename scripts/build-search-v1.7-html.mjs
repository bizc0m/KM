...
const outputFile = join(root, "search-v1.7.html");
...
function themeOf(title, tags, type) {
  ...
  if (/rouge|pentest|red team|offensive|osint|security|vulnerability|anti detection/.test(text)) return "security";
  if (/rss|source|authors|monitoring|twitter|x\b|openrss|nitter/.test(text)) return "source";
  if (/github|code|devtools|editor|coding|api|llm|agent|agents|ia|ai/.test(text)) return "ai";
  if (/research|paper|citation/.test(text)) return "research";
  return "tools";
}
...
.meta{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:6px}.tag{font-size:9px;text-transform:uppercase;font-weight:900;padding:3px 6px;border:1px solid var(--line);background:var(--soft)}.tag.hot{background:var(--red-soft);border-color:#edb8b8;color:#8f0000}.tag.ai{background:#eef4fb;border-color:#b8cce0;color:var(--blue)}.tag.warn{background:#fff6dd;border-color:#e6cf89;color:#6f5100}.tag.security{background:#fff0ef;border-color:#edb8b8;color:#8f0000}.tag.source{background:#fff6dd;border-color:#e6cf89;color:#6f5100}.tag.research{background:#edf3ff;border-color:#b9c9ee;color:#244678}.tag.tools{background:#eef7ee;border-color:#bad8ba;color:#225522}.tag.github-link{background:#fff!important;color:var(--ink)!important;border-color:var(--ink)!important;text-decoration:none}
...
<main class="app">
  ...
  <footer class="footer"><span><b>KM Search v1.7</b> · HTML autonome · vue publique nettoyee</span><span>${index.length} fiches · ${generatedAt}</span></footer>
</main>
...
function markdown(md){const lines=String(md||"").split("\\n");let out="", inList=false, inTable=false;const close=()=>{if(inList){out+="</ul>";inList=false}if(inTable){out+="</tbody></table>";inTable=false}};for(const line of lines){if(/^\\|(.+)\\|$/.test(line)&&!/^\\|\\s*-/.test(line)){if(!inTable){close();out+="<table><tbody>";inTable=true}out+="<tr>"+line.slice(1,-1).split("|").map(c=>"<td>"+esc(c.trim())+"</td>").join("")+"</tr>";continue}if(/^\\|\\s*-/.test(line))continue;if(inTable&&!/^\\|/.test(line))close();if(/^###\\s+/.test(line)){close();out+="<h3>"+esc(line.replace(/^###\\s+/,""))+"</h3>";continue}if(/^##\\s+/.test(line)){close();out+="<h2>"+esc(line.replace(/^##\\s+/,""))+"</h2>";continue}if(/^#\\s+/.test(line)){close();out+="<h1>"+esc(line.replace(/^#\\s+/,""))+"</h1>";continue}if(/^[-*]\\s+/.test(line)){if(!inList){close();out+="<ul>";inList=true}out+="<li>"+esc(line.replace(/^[-*]\\s+/,""))+"</li>";continue}if(!line.trim()){close();continue}close();out+="<p>"+esc(line).replace(/\`([^\`]+)\`/g,"<code>$1</code>")+"</p>"}close();return out}
...
function cardHtml(item){const active=item.id===activeId, sel=selected.has(item.id), gh=item.github?'<a class="tag github-link" href="'+esc(item.github)+'" target="_blank" rel="noopener noreferrer">GITHUB</a>':"";return '<article class="news-card '+(active?"active":"")+' '+(sel?"selected":"")+'" data-id="'+esc(item.id)+'"><div class="check" data-action="select" title="Selectionner"></div><div><span class="date-chip">'+esc(item.integratedAt||"date n/a")+'</span><h3 class="news-title" data-action="open">'+highlight(item.title)+'</h3><div class="meta">'+gh+'<span class="tag '+statusClass(item.status)+'">'+esc(item.status)+'</span><span class="tag '+esc(item.theme)+'">'+esc(item.theme)+'</span>'+item.tags.filter(t=>norm(t)!=="watch").slice(0,3).map(t=>'<span class="tag '+esc(item.theme)+'">'+highlight(t)+'</span>').join("")+'</div></div></article>'}
...
byId("copySelection").onclick=()=>{const items=KM_INDEX.filter(i=>selected.has(i.id));copy("# KM Search v1.7 selection\\n\\n"+(items.length?items:[activeItem()]).map(itemMarkdown).join("\\n")).then(()=>toast("Selection copiee"))};
byId("exportSelection").onclick=()=>{const items=KM_INDEX.filter(i=>selected.has(i.id));download("km-search-v1.7-selection.md","# KM Search v1.7 selection\\n\\n"+(items.length?items:[activeItem()]).map(itemMarkdown).join("\\n"))};
byId("exportJson").onclick=()=>download("km-search-v1.7-index.json",JSON.stringify(KM_INDEX,null,2));
