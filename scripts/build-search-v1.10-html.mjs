...
<main class="app">
  ...
  <footer class="footer"><span><b>KM Search v1.10</b> · HTML autonome · vue publique nettoyee</span><span>${index.length} fiches · ${generatedAt}</span></footer>
</main>
...
function cardHtml(item){const active=item.id===activeId, sel=selected.has(item.id), gh=item.github?'<a class="tag github-link" href="'+esc(item.github)+'" target="_blank" rel="noopener noreferrer">GITHUB</a>':"";return '<article class="news-card '+(active?"active":"")+' '+(sel?"selected":"")+'" data-id="'+esc(item.id)+'"><div class="check" data-action="select" title="Selectionner"></div><div><div class="title-row"><span class="date-chip">'+esc(item.integratedAt||"date n/a")+'</span>'+gh+'<h3 class="news-title" data-action="open">'+highlight(item.title)+'</h3></div><div class="meta"><span class="tag '+statusClass(item.status)+'">'+esc(item.status)+'</span><span class="tag '+esc(item.theme)+'">'+esc(item.theme)+'</span>'+item.tags.filter(t=>norm(t)!=="watch").slice(0,3).map(t=>'<span class="tag '+esc(themeOfTag(t,item.theme))+'">'+highlight(t)+'</span>').join("")+'</div></div></article>'}
...
byId("copySelection").onclick=()=>{const items=KM_INDEX.filter(i=>selected.has(i.id));copy("# KM Search v1.10 selection\\n\\n"+(items.length?items:[activeItem()]).map(itemMarkdown).join("\\n")).then(()=>toast("Selection copiee"))};
byId("exportSelection").onclick=()=>{const items=KM_INDEX.filter(i=>selected.has(i.id));download("km-search-v1.10-selection.md","# KM Search v1.10 selection\\n\\n"+(items.length?items:[activeItem()]).map(itemMarkdown).join("\\n"))};
byId("exportJson").onclick=()=>download("km-search-v1.10-index.json",JSON.stringify(KM_INDEX,null,2));
