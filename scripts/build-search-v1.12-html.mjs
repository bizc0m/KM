...
function inlineNoteHtml(item){const concepts=item.concepts.filter(c=>c.parent).slice(0,4).map(c=>tagButton(c.label,themeOfTag(c.label,item.theme))).join("");const tags=item.tags.filter(t=>norm(t)!=="#watch"&&norm(t)!=="watch"&&!sameTagTheme(t,item.theme)&&!(isSensitiveStatus(item.status)&&/^#?(rouge|sensible|sensitive)$/i.test(t))).slice(0,4).map(t=>tagButton(t,themeOfTag(t,item.theme))).join("");return '<div class="inline-note"><div class="inline-head">'+tagButton(statusLabel(item.status),statusClass(item.status))+'<h4 class="inline-title">'+highlight(item.title)+'</h4></div><div class="source-strip"><span>'+esc(item.integratedAt||"date n/a")+'</span><span>'+esc(item.path)+'</span><span>'+esc(item.folderLabel)+'</span><span>'+esc(item.themeLabel)+'</span>'+concepts+tags+'</div><p>'+esc(item.summary)+'</p><div class="inline-actions"><a class="btn dark" href="'+esc(item.path)+'" target="_blank" rel="noopener noreferrer">Ouvrir .md</a><button class="btn" data-action="copy">Copier MD</button><button class="btn" data-action="collapse">Replier</button><button class="btn red" data-action="select">Selection</button></div></div>'}
