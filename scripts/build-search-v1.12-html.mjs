...
<main class="app">
  ${globalMenu("search")}
  <div class="topbar"><div class="brandblock"><div class="brand"><span>KM</span> Search <b class="version-badge">v1.12</b><a class="rss-version" href="${config.sources?.raindropFeedFallback || "#"}" target="_blank" rel="noopener noreferrer" aria-label="RSS" title="RSS"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 11a9 9 0 0 1 9 9"></path><path d="M4 4a16 16 0 0 1 16 16"></path><circle cx="5" cy="19" r="1"></circle></svg></a></div><div class="subtitle">Recherche · Watch · RSS/Raindrop · fiches Markdown</div></div><div class="statsbar" id="statsbar"></div><div class="tool-actions"><button class="btn dark icon-btn" id="navHome" aria-label="Home" title="Home"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 11l9-8 9 8"></path><path d="M5 10v10h14V10"></path><path d="M9 20v-6h6v6"></path></svg></button><button class="btn icon-btn" id="navBack" aria-label="Retour" title="Retour"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 18l-6-6 6-6"></path></svg></button><button class="btn icon-btn" id="navForward" aria-label="Avancer" title="Avancer"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 6l6 6-6 6"></path></svg></button><a class="btn dark" href="search-v1.12.html">Recherche</a><button class="btn" id="watchShortcut">Watch</button><a class="btn" href="${config.sources?.raindropPublicPage || "#"}" target="_blank" rel="noopener noreferrer">Raindrop</a><a class="btn" href="kprompt.html">Kprompt</a><a class="btn" href="http://127.0.0.1:8767/">Veille locale</a><button class="btn dark" id="toggleFicheTop">Afficher fiche</button><button class="btn" id="copyShare">Copier lien</button><button class="btn dark" id="exportJson">JSON</button></div></div>
...
function ficheTagsLine(item,limit){const chips=visibleTags(item).slice(0,limit||8).map(tag=>tagButton(tag,themeOfTag(tag,item.theme))).join("");return chips?'<div class="info-line"><span class="info-label">Tags</span>'+chips+'</div>':""}
