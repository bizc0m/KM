...
function functionTagsLine(item,limit){const seen=new Set();const chips=[];const add=(label,cls)=>{const key=chipKey(label);if(!key||seen.has(key))return;seen.add(key);chips.push(tagButton(label,cls||themeOfTag(label,item.theme)))};functionKeywordEntries(item).forEach(({label,cls})=>add(label,cls));visibleTags(item).slice(0,limit||10).forEach(tag=>add(tag,themeOfTag(tag,item.theme)));return chips.length?'<div class="info-line"><span class="info-label">Tags</span>'+chips.join("")+'</div>':""}
...
function cardHtml(item){const active=item.id===activeId, sel=selected.has(item.id);return '<article class="news-card '+(active?"active":"")+' '+(sel?"selected":"")+'" data-id="'+esc(item.id)+'"><div><div class="title-row">'+statusBadge(item)+'<h3 class="news-title" data-action="open">'+highlight(item.title)+'</h3></div>'+cardInfoLines(item)+cardPrimaryLinks(item)+functionTagsLine(item,10)+topicsLine(item,12)+cardActions(item)+'</div></article>'}
...
function applyHashState(){applyingHash=true;try{const raw=location.hash.replace(/^#/,"");if(!raw){resetHomeState();return}if(raw.startsWith("/")){let slug=raw.slice(1);if(slug.endsWith("+"))slug=slug.slice(0,-1);byId("search").value=decodeSearchSlug(slug);byId("tagFilter").value="";byId("sort").value="date";byId("statusFilter").value="all";byId("folderFilter").value="all";byId("themeFilter").value="all";byId("conceptFilter").value="all";setFicheCollapsed(true);renderAll();return}const hash=new URLSearchParams(raw);if(hash.get("q")!==null)byId("search").value=hash.get("q")||"";if(hash.get("status"))byId("statusFilter").value=hash.get("status");if(hash.get("tag")!==null)byId("tagFilter").value=hash.get("tag")||"";if(hash.get("sort"))byId("sort").value=hash.get("sort");renderFilters();if(hash.get("folder"))byId("folderFilter").value=hash.get("folder");if(hash.get("theme"))byId("themeFilter").value=hash.get("theme");if(hash.get("concept"))byId("conceptFilter").value=hash.get("concept");if(hash.get("fiche")&&KM_INDEX.some(i=>i.id===hash.get("fiche"))){activeId=hash.get("fiche");setFicheCollapsed(false)}else setFicheCollapsed(true);renderAll()}finally{applyingHash=false}}
document.addEventListener("click",e=>{if(e.target.id==="toggleFiche"||e.target.id==="toggleFicheHeader"||e.target.id==="toggleFicheTop"){const collapsed=!byId("workspace").classList.contains("fiche-collapsed");setFicheCollapsed(collapsed);renderAll();toast(collapsed?"Liste affichee":"Fiche ouverte");return}if(e.target.id==="openFiche"){setFicheCollapsed(false);renderAll();toast("Fiche ouverte");return}const tagBtn=e.target.closest('[data-action="tag"]');if(tagBtn){byId("tagFilter").value=tagBtn.dataset.tag||"";renderAll();toast("Tag filtre: "+(tagBtn.dataset.tag||""));return}const topicBtn=e.target.closest(".topic");if(topicBtn){topic=topicBtn.dataset.topic;renderAll();return}const card=e.target.closest(".news-card");if(card&&e.target.dataset.action==="select"){toggleSelection(card.dataset.id);return}if(card&&(e.target.dataset.action==="open"||!e.target.closest("a,button,.check"))){activeId=card.dataset.id;setFicheCollapsed(false);renderAll();return}if(e.target.id==="archiveActive"){archiveActive();return}if(e.target.id==="copyActive"){copy(itemMarkdown(activeItem())).then(()=>toast("Fiche active copiee"));return}if(e.target.id==="selectActive"){toggleSelection(activeItem().id);return}});
...
byId("copyRss").onclick=()=>copy("${config.sources?.raindropFeedFallback || ""}").then(()=>toast("RSS source copie"));
