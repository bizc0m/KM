...
function cardPrimaryLinks(item){const github=cardGithubUrl(item);const webSource=cardWebSource(item);const githubChip=github?'<a class="tag github-link" href="'+esc(github)+'" target="_blank" rel="noopener noreferrer">GitHub</a>':'<span class="tag github-link missing-link">GitHub</span>';const webChip=webSource?'<a class="tag product-link" href="'+esc(webSource.url)+'" target="_blank" rel="noopener noreferrer">Link</a>':"";const seenOrigins=new Set();const origins=cardOriginSources(item,webSource).map(source=>{const key=norm(source.url).replace(/\\/$/,"");if(!source.url||seenOrigins.has(key))return"";seenOrigins.add(key);return '<a class="tag origin-link" href="'+esc(source.url)+'" target="_blank" rel="noopener noreferrer">Origine</a>'}).join("");return '<div class="source-strip card-primary-strip">'+githubChip+webChip+origins+'</div>'}
function cardFact(item){return item.summary||item.type||item.title}
function cardInfoLines(item){const desc=cardFact(item);return desc?'<div class="info-line card-description-strip"><div class="card-info-row"><span class="info-label">Fait</span><span class="card-description">'+highlight(desc)+'</span></div></div>':""}
function cardActions(item){return""}
function cardHtml(item){const active=item.id===activeId, sel=selected.has(item.id);return '<article class="news-card '+(active?"active":"")+' '+(sel?"selected":"")+'" data-id="'+esc(item.id)+'"><div><div class="title-row">'+statusBadge(item)+'<h3 class="news-title" data-action="open">'+highlight(item.title)+'</h3></div>'+cardPrimaryLinks(item)+cardInfoLines(item)+topicsLine(item,8)+functionTagsLine(item,6)+cardActions(item)+'</div></article>'}
