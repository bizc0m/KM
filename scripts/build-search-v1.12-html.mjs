...
function githubTopicsFrom(content) {
  ...
  for (const line of content.split("\n")) {
    const bullet = line.match(/topics(?:\s+releves)?\s*:\s*(.+)$/i);
    ...
    if (/^\|(.+)\|$/.test(line) && !/^\|\s*-/.test(line)) {
      ...
      const index = tableHeaders.findIndex((header) => header === "topics-releves" || header === "topics");
...
function cardPrimaryLinks(item){const github=cardGithubUrl(item);const webSource=cardWebSource(item);const githubChip=github?'<a class="tag github-link" href="'+esc(github)+'" target="_blank" rel="noopener noreferrer">GitHub</a>':'<span class="tag github-link missing-link">GitHub</span>';const webChip=webSource?'<a class="tag product-link" href="'+esc(webSource.url)+'" target="_blank" rel="noopener noreferrer">Link</a>':"";const origins=cardOriginSources(item,webSource);const originChips=origins.map((source,index)=>{const label=origins.length>1?"Source "+(index+1):"Source";return '<a class="tag origin-link" href="'+esc(source.url)+'" target="_blank" rel="noopener noreferrer">'+label+'</a>'}).join("");return '<div class="source-strip card-primary-strip title-links">'+githubChip+webChip+originChips+'</div>'}
function cardFact(item){return item.summary||item.type||item.title}
function cardInfoLines(item){const desc=cardFact(item);return desc?'<div class="card-description-strip"><span class="card-description">'+highlight(desc)+'</span></div>':""}
function cardActions(item){return""}
function cardHtml(item){const active=item.id===activeId, sel=selected.has(item.id);return '<article class="news-card '+(active?"active":"")+' '+(sel?"selected":"")+'" data-id="'+esc(item.id)+'"><div><div class="title-row">'+statusBadge(item)+'<h3 class="news-title" data-action="open">'+highlight(item.title)+'</h3>'+cardPrimaryLinks(item)+'</div>'+cardInfoLines(item)+topicsLine(item,8)+functionTagsLine(item,6)+cardActions(item)+'</div></article>'}
